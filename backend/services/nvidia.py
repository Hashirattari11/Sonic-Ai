...
# Free-tier models on NVIDIA Build (no credit card needed).
DEFAULT_MODEL = "nvidia/llama-3.3-nemotron-super-70b"
FALLBACK_MODELS = [
...
class NVIDIAService:
    ...
    def _extract_content(self, body: str) -> str:
        """Parse OpenAI-style JSON or SSE (event-stream) chunks."""
        body = body.strip()
    ...
    async def verify_key(self) -> bool:
        try:
            ...
            if status == 200:
                return True
            # 429 usually means the free key is rate-limited, not invalid.
            if status == 429:
                return True
            return False
        except Exception:
    ...
    async def chat(self, message: str, history: list, memory: dict) -> str:
        ...
        if last_error and "quota" in last_error.lower():
            return ("Your NVIDIA API key has no available quota. Get a free key at "
                    "https://build.nvidia.com - it includes free credits.")
        return ("SONIC AI could not reach the NVIDIA AI service. The free model may be "
        ...
        for model in models:
            for attempt in range(2):
                try:
                    ...
                    last_error = str(e)
                    break
        raise Exception(f"NVIDIA raw chat failed: {last_error}")
